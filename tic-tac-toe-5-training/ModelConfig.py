...
class ModelConfig:
    ...
    def clone(self, name=None, input_strategy=None):
        if name is None:
            name = self.model_name
        if input_strategy is None:
            input_strategy = self.input_strategy_number

        return ModelConfig(
            [*self.hidden_structure],
            input_strategy,
            self.output_strategy_number,
            self.use_bias,
            self.use_attention,
            name
        )
    ...
    def save_model(self, dir):
        ...
        model_filepath = os.path.join(dir, self.model_name, "model.pkl")
    ...
    def load_model(dir, model_name):
        model_filepath = os.path.join(dir, model_name, "model.pkl")
    ...
    def save_weights(self, weights_data: list[list]):
        ...
        self.save_info(dir, 'weights', weights_data)

    # Загрузка весов
    def load_weights(self, dir) -> list[list]:
        """Загрузка весов из файла 'weights.npy'."""
        return self.load_info(dir, 'weights')

    # Сохранение информации
    def save_info(self, dir, info_title, info_data):
        """Сохранение информации в файл с заголовком info_title."""
        info_filepath = os.path.join(dir, self.model_name, f"{info_title}.pkl")
        os.makedirs(os.path.dirname(info_filepath), exist_ok=True)
        with open(info_filepath, 'wb') as file:
            pickle.dump(info_data, file)

    # Загрузка информации
    def load_info(self, dir, info_title):
